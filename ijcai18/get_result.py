...
def main(args): 
    ...
    with tf.Graph().as_default():
    	...
    	# Load the model
    	    ...
    	    feature_size = embeddings.get_shape()[1]
    	...
    	# features_arr = np.zeros((image_num, feature_size))
    	    ...
    	    for i in range(batch_num):
    	        features_arr = np.zeros((batch_size, feature_size))
...
#    	        features_arr = sess.run(features[0:end_index-start_index,0,0,:], feed_dict=feed_dict)
    	        features_arr = sess.run(embeddings[0:end_index-start_index,:] , feed_dict=feed_dict)
